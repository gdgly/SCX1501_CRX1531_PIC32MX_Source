...
void HA_uart_send_APP(void)
{
    ...
    if((FLAG_TIME_No_response==1)&&(TIME_No_response==0)){
        ...
        HA_uart_app[13]=b0.IDB[2];
        HA_uart_app[15]=0xFF;
        if((FLAG_AUTO_SEND_START==1)&&(FG_send_Faile_again==0)) {FG_send_Faile_again=1;TIME_alarm_AUTO=350; FLAG_HA_Inquiry=1;DATA_Packet_Control_0=0x00; FLAG_AUTO_SEND_ok=1;}    //2015.1.30追加修改自动某ID发送一次失败，追加再发送一次
