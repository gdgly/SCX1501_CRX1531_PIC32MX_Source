...
#define BaudRate 64
const UINT8 wifi_uart[8]={0xBB,0x00,0x06,0x80,0x00,0x00,0x05,0x00};
...
void __ISR(_UART_1_VECTOR,ipl3)Uart1Handler(void)
{
    ...
    if((FLAG_UART_0xBB==0)&&(UART_DATA_cnt>=1)){
        if((UART_DATA_buffer[UART_DATA_cnt-1]==0xBB)&&(UART_DATA_buffer[UART_DATA_cnt]==0x00)){UART_DATA_cnt=1;UART_DATA_buffer[0]=0xBB;UART_DATA_buffer[1]=0x00;FLAG_UART_0xBB=1;}
    }
    UART_DATA_cnt++;
    if(UART_DATA_cnt>=15){
        UART_DATA_cnt=0;
